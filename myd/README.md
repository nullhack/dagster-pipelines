...
| Name | Description |
|-|-|
| `myd/` | A Python module that contains code for your Dagster repository |
| `myd_tests/` | A Python module that contains tests for `myd` |
...
pyenv virtualenv $PYTHON_VERSION myd
pyenv activate myd
```
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name myd python=PYTHON_VERSION
conda activate myd
```
...
Tests can be found in `myd_tests` and are run with the following command:

```bash
pytest myd_tests
```

As you create Dagster solids and pipelines, add tests in `myd_tests/` to check that your
